...
interface GatherControllerDependencies {
  ...
  onPickupForRaskhod?(keyword: string): void;
}
...
export function createGatherController(deps: GatherControllerDependencies) {
  ...
  function processLine(line: string): void {
    ...
    const puddleMatch = PUDDLE_REGEXP.exec(line);
    if (puddleMatch) {
      pickup(lastWord(puddleMatch[1]), "расход");
      return;
    }
  }

  function pickup(item: string, targetBag?: string): void {
    const keyword = item.slice(0, 6);
    const dest = targetBag ?? bag;
    if (dest === "расход") {
      deps.onPickupForRaskhod?.(keyword);
    }
    deps.onLog(`[gather] взя ${keyword}; полож ${keyword} ${dest}`);
    deps.sendCommand(`взя ${keyword}`);
    deps.sendCommand(`полож ${keyword} ${dest}`);
  }
