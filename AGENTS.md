...
## MUD-команды — правила работы через bylins-client

### Ключевые слова вместо полных названий

МУД разбирает команды по **первому ключевому слову** предмета. Прилагательные в русском склонении часто не распознаются. Правила:

- ✅ `надеть кольцо` — работает (первое существительное)
- ✅ `снять ремень` — работает
- ❌ `надеть змеиное кольцо` — не работает (МУД ругается «странная анатомия»)
- ❌ `надеть накидку бунтаря` — не работает если предмет в контейнере
- ✅ `взять накидку торба` — взять предмет из контейнера по ключевому слову
- ✅ `взять накидку сундук` — то же для сундука

**Если предмет в контейнере (торба, сундук)** — МУД не видит его напрямую для `надеть`. Сначала `взять <предмет> <контейнер>`, потом `надеть <предмет>`.

**Если слот уже занят** — сначала `снять <что надето>`, потом `надеть <новое>`.

### Пагинация в справке

МУД выводит длинные справки постранично с пагинатором. Через MCP (`send_command`) листать нельзя — Enter не работает как продолжение. Пагинатор зависает.

- ✅ Запрашивать конкретную страницу: `справка 2.имяраздела` (работает не всегда)
- ✅ Читать первую страницу — она всегда приходит полностью
- ❌ Не пытаться листать через пустую строку или Enter в `send_command`
- После застрявшего пагинатора — отправить `q` для выхода

### Полезные команды для анализа

```
надеть <ключслово>          # надеть предмет
снять <ключслово>           # снять предмет
взять <предмет> <контейнер> # взять из контейнера
осмотреть <предмет>         # краткое описание
определить <предмет>        # характеристики (требует заклинание)
справка <тема>              # игровая справка (без пробелов в имени)
score                       # характеристики персонажа
инвентарь                   # список вещей
одет                        # что надето
```

### Знания об аффектах предметов

- **`не.сбежать`** — блокирует побег **носителя** с поля боя, а не врага. Опасно для выживания, если персонаж в опасности.
...
> **ВАЖНО:** Для дебага и отладки **ВСЕГДА читать лог через файл** (`/var/log/bylins-bot/mud-traffic.log`).
> `bylins-client_get_logs` (MCP) показывает только события скриптов бота, а не полный MUD-трафик — для диагностики непригоден.
...
This project is indexed by GitNexus as **bylins-bot** (1236 symbols, 3010 relationships, 103 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
